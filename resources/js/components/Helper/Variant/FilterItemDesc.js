...
const filterItemDesc = (variantsData, variantId) => {
    ...
    const filterItemAttributes = variantsData.filter(
        index => index.variants.items_id == variantId
    );

    for (var keys in variantInit) {
        variantDict.push({
            key: keys,
            value: filterItemAttributes.filter(index =>
                index.variants.name.includes(variantInit[keys])
            )
        });
    }
            ...
            formGroup.push(
                variantsMap.map(variant => (
                    <FormControlLabel
                        ...
                        name="Color"
                        value={variant.name}
                        control={<Radio value={variant.name} size="small" />}
                        label={
                            <div
                                className={`${variant.alias} border rounded w-6 h-6`}
                            />
                        }
                    />
                ))
